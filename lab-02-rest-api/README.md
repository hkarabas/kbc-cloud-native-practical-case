...
To make sure the Mobile app can start their implementation asap we have decided to implement these APIs using dummy values before we integrate with the 3rd party cocktailDto and meal APIs.
...
* it must return the cocktailDto resource(s) in ``json`` format
...
* hard-code the 2 cocktailDto resources from the above sample and return them from the method you just declared
